...
export const OxygenRecievingPage = () => {
  ...
  const handleNavigate = (uid) => {
    navigate(`/oxygen-management/recievings/open/${uid}`);
  };
  ...
  const handleFetchData = async () => {
    ...
    try {
      ...
      if (result.status === 200 || result.status === 8000) {
        setOxygenRecievings(result.data);
  ...
  const handleDelete = async (oxygenRecieving = null) => {
    if (!oxygenRecieving) {
    ...
    try {
      ...
      if (confirmation.isConfirmed) {
        const result = await deleteOxygenRecieving(oxygenRecieving.uid);
        if (result.status === 200 || result.status === 8000) {
          Swal.fire(
            "Process Completed!",
    ...
    setSelectedOxygenRecieving(null); // Reset selected OxygenRecieving after deletion
  };

  // Fetch OxygenRecievings on initial load
  useEffect(() => {
  ...
  return (
    <OxygenRecievingContext.Provider
      value={{
        handleFetchData,
        selectOxygenRecieving,
        setSelectedOxygenRecieving,
      }}
    >
      <h4 className="py-3 mb-4">
        <span className="text-muted fw-light">Setting /</span> Oxygen Recievings
      </h4>
      <div className="card">
        <div className="d-flex justify-content-between align-items-center card-header">
          <h5 className="mb-0">All Oxygen Recieving</h5>
          <OxygenRecievingsModal
            title="View Oxygen Recievings"
            ...
            <table className="table table-hover table-align-middle mb-0 table-bordered">
              <thead style={{ backgroundColor: "#f1f1f1" }}>
                <tr>
                  ...
                  <th>Location</th>
                  <th>Receiving&nbsp;No</th>
                  <th style={{ width: "250px" }}>Recieved&nbsp;By</th>
              ...
              <tbody className="table-border-bottom-0">
                {loading ? (
                      ...
                      <div className="col-md-12 col-lg-12 col-sm-12 p-2">
                        ...
                        <center className="mt-1">
                          <h6 className="text-muted">
                            Fetching Oxygen Recievings
                          </h6>
                        </center>
                      </div>
                    </td>
                  </tr>
                ) : error || oxygenRecievings.length === 0 ? (
                  <tr>
                    <td colSpan="100%">
                  ...
                  oxygenRecievings.map((dataRow, index) => (
                    <tr key={dataRow.uid}>
                      ...
                      <td className="fw-medium">{dataRow.receiving_number}</td>
                      <td className="fw-medium">{dataRow.received_by?.name}</td>
