...
import "animate.css";
import { deleteOxygenUsage, getOxygenUsages } from "./Queries";
import { OxygenUsageContext } from "../../../../utils/context";
import OxygenUsagesModal from "./Modal";
import { useNavigate } from "react-router-dom";
import ReportModal from "./ReportModal";

export const OxygenUsagePage = () => {
  const pageSizeData = [5, 10, 20, 50, 70, 100];
  const FilterData = ["All", "New", "Verified", "Partial Verified", "Rejected"];

  const [filter, setFilter] = useState("ALL");
  const navigate = useNavigate();
  const [oxygenUsages, setOxygenUsages] = useState([]);
  const [loading, setLoading] = useState(true);
  const [error, setError] = useState(null);
  const [selectOxygenUsage, setSelectedOxygenUsage] = useState(null);
  ...
  const handleNavigate = (uid) => {
    navigate(`/oxygen-management/usages/open/${uid}`);
  };
  ...
  const handleFetchData = async () => {
    ...
    try {
      ...
      if (result.status === 200 || result.status === 8000) {
        ...
        setError(true);
        showToast("No Oxygen Usage Found", "warning", "Fetch Completed");
      }
    } catch (err) {
      setError(true);
      showToast("Unable to Fetch Oxygen Usages", "warning", "Failed");
    } finally {
      setLoading(false);
    }
  };

  const handleDelete = async (oxygenUsage = null) => {
    if (!oxygenUsage) {
      Swal.fire("Error!", "Unable to Select this Oxygen Usage.", "error");
      return;
    }
    ...
    try {
      ...
      if (confirmation.isConfirmed) {
        const result = await deleteOxygenUsage(oxygenUsage.uid);
        if (result.status === 200 || result.status === 8000) {
          Swal.fire(
            "Process Completed!",
            "The Oxygen Usage has been deleted.",
            "success"
          );
          handleFetchData();
        } else {
          console.error("Error deleting Oxygen Usage:", result);
    ...
    setSelectedOxygenUsage(null); // Reset selected OxygenUsage after deletion
  };

  // Fetch OxygenUsages on initial load
  useEffect(() => {
  ...
  return (
    ...
    >
      <h4 className="py-3 mb-4">
        <span className="text-muted fw-light">Setting /</span> Oxygen Usages
      </h4>
      <div className="card">
        <div className="d-flex justify-content-between align-items-center card-header">
          <h5 className="mb-0">All Oxygen Usage</h5>

          <div style={{ width: "300px" }}>
            <OxygenUsagesModal
              title="View Oxygen Usages"
            ...
            >
              <i className="bx bxs-file-pdf me-1"></i> Print Report
            </button>
          </div>
        </div>
        ...
        <div className="card-body ">
          <div className="d-flex justify-content-between align-items-center mb-2 animate__animated animate__fadeInDown animate__faster">
            <div className="d-flex align-items-center col-md-8 col-sm-6">
              ...
              <label
                className="text-sm font-medium  me-2 mb-0"
            ...
            <table className="table table-hover table-align-middle mb-0 table-bordered">
              <thead style={{ backgroundColor: "#f1f1f1" }}>
                <tr>
                  ...
                  <th>Location</th>
                  <th style={{ width: "250px" }}>Registered&nbsp;By</th>
              ...
              <tbody className="table-border-bottom-0">
                {loading ? (
                  ...
                  oxygenUsages.map((dataRow, index) => (
                    <tr key={dataRow.uid}>
                      <td>{(currentPage - 1) * pageSize + index + 1}</td>
                      <td className="fw-medium">
                        {(() => {
                          const d = new Date(dataRow.date);
                      ...
                      <td className="fw-medium">{dataRow.location?.name}</td>
                      <td className="fw-medium">{dataRow.used_by?.name}</td>
                      <td className="fw-medium">
                        <span
                          className={
                            dataRow.status === "NEW"
