...
export const OxygenLocationPage = () => {
  ...
  const handleView = (uid) => {
    navigate(`/oxygen-management/locations/open/${uid}`);
  };

  const handleUsage = (uid) => {
    navigate(`/oxygen-management/locational-oxygen-usages/open/${uid}`);
  };
  ...
  const handleDelete = async (oxygenLocation = null) => {
    if (!oxygenLocation) {
    ...
    try {
      ...
      if (confirmation.isConfirmed) {
        const result = await deleteOxygenLocation(oxygenLocation.uid);
        if (result.status === 200 || result.status === 8000) {
          Swal.fire(
            "Process Completed!",
  ...
  // Fetch OxygenLocations on initial load
  useEffect(() => {
  ...
  return (
    ...
    >
      <h4 className="py-3 mb-4">
        <span className="text-muted fw-light">Setting /</span> Oxygen Locations
      </h4>
      <div className="card">
        <div className="d-flex justify-content-between align-items-center card-header">
          <h5 className="mb-0">All Oxygen Location</h5>
          <OxygenLocationsModal
            title="View Oxygen Locations"
            ...
            <table className="table table-hover table-align-middle mb-0 table-bordered">
              ...
              <tbody className="table-border-bottom-0">
                {loading ? (
                      ...
                      <div className="col-md-12 col-lg-12 col-sm-12 p-2">
                        ...
                        <center className="mt-1">
                          <h6 className="text-muted">
                            Fetching Oxygen Locations
                          </h6>
                        </center>
                      </div>
                    </td>
                  </tr>
                ) : error || oxygenLocations.length === 0 ? (
                  <tr>
                    <td colSpan="100%">
                  ...
                  oxygenLocations.map((dataRow, index) => (
                    <tr key={dataRow.uid}>
                      <td>{(currentPage - 1) * pageSize + index + 1}</td>
                      <td
                        className="fw-medium text-info cursor-pointer"
                        onClick={(e) => {
                          e.preventDefault();
                          handleView(dataRow.uid);
                        }}
                      >
                        {dataRow.name}
                      </td>
                      <td className="fw-medium">{dataRow.code}</td>
                      <td className="fw-medium">{dataRow.type}</td>
                        ...
                        <div className="dropdown">
                          ...
                          <div className="dropdown-menu">
                            ...
                            <button
                              ...
                              onClick={(e) => {
                                e.preventDefault();
                                handleUsage(dataRow.uid);
                              }}
                            >
                              <i className="bx bx-stats me-1"></i> Usage
                            </button>
                          </div>
                        </div>
                      </td>
