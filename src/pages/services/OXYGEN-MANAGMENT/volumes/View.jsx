...
import "animate.css";
import { deleteOxygenVolume, getOxygenVolumes } from "./Queries";
import { OxygenVolumeContext } from "../../../../utils/context";
import OxygenVolumesModal from "./Modal";

export const OxygenVolumePage = () => {
  const pageSizeData = [5, 10, 20, 50, 70, 100];
  const [oxygenVolumes, setOxygenVolumes] = useState([]);
  const [loading, setLoading] = useState(true);
  const [error, setError] = useState(null);
  const [selectOxygenVolume, setSelectedOxygenVolume] = useState(null);
  ...
  const handleFetchData = async () => {
    ...
    setError(null);
    try {
      const result = await getOxygenVolumes({
        search: searchQuery,
        pagination: {
          page: currentPage,
          page_size: pageSize,
      ...
      if (result.status === 200 || result.status === 8000) {
        ...
        setError(true);
        showToast("No Oxygen Volume Found", "warning", "Fetch Completed");
      }
    } catch (err) {
      setError(true);
      showToast("Unable to Fetch Oxygen Volumes", "warning", "Failed");
    } finally {
      setLoading(false);
    }
  };

  const handleDelete = async (oxygenVolume = null) => {
    if (!oxygenVolume) {
      Swal.fire("Error!", "Unable to Select this Oxygen Volume.", "error");
      return;
    }
    ...
    try {
      ...
      if (confirmation.isConfirmed) {
        const result = await deleteOxygenVolume(oxygenVolume.uid);
        if (result.status === 200 || result.status === 8000) {
          Swal.fire(
            "Process Completed!",
            "The Oxygen Volume has been deleted.",
            "success"
          );
          handleFetchData();
        } else {
          console.error("Error deleting Oxygen Volume:", result);
    ...
    setSelectedOxygenVolume(null); // Reset selected OxygenVolume after deletion
  };

  // Fetch OxygenVolumes on initial load
  useEffect(() => {
  ...
  return (
    <OxygenVolumeContext.Provider
      value={{
        handleFetchData,
        selectOxygenVolume,
        setSelectedOxygenVolume,
      }}
    >
      <h4 className="py-3 mb-4">
        <span className="text-muted fw-light">Setting /</span> Oxygen Volumes
      </h4>
      <div className="card">
        <div className="d-flex justify-content-between align-items-center card-header">
          <h5 className="mb-0">All Oxygen Volume</h5>
          <OxygenVolumesModal
            title="View Oxygen Volumes"
            onClose={() => setSelectedOxygenVolume(null)}
          />
        </div>
            ...
            <table className="table table-hover table-align-middle mb-0 table-bordered">
              ...
              <tbody className="table-border-bottom-0">
                {loading ? (
                      ...
                      <div className="col-md-12 col-lg-12 col-sm-12 p-2">
                        ...
                        <center className="mt-1">
                          <h6 className="text-muted">
                            Fetching Oxygen Cylinders
                          </h6>
                        </center>
                      </div>
                    </td>
                  </tr>
                ) : error || oxygenVolumes.length === 0 ? (
                  <tr>
                    <td colSpan="100%">
                  ...
                  oxygenVolumes.map((dataRows, index) => (
                    <tr key={dataRows.uid}>
                      <td>{(currentPage - 1) * pageSize + index + 1}</td>
                      <td className="fw-medium">{dataRows.name}</td>
                      <td className="fw-medium">{dataRows.volume}</td>
                        ...
                        <div className="dropdown">
                          ...
                          <div className="dropdown-menu">
                            <a
                              ...
                              href="#"
                              onClick={() => {
                                setSelectedOxygenVolume(dataRows);
                              }}
