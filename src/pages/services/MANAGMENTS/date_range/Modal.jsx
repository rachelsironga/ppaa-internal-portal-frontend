...
import { createUpdateDateRange } from "./Queries";
import { DateRangeContext } from "../../../../utils/context";
import dateRangeData from "../../../../data/dateRangeData.json";
import Select from "react-select";

const DateRangeModal = () => {
  const { handleFetchData, selectDateRange, setSelectedDateRange } =
    useContext(DateRangeContext);
  const [errors, setOtherError] = useState({});
  const initialValues = {
    name: selectDateRange?.name || "",
    type: selectDateRange?.type || "",
    value: selectDateRange?.value || "",
    order: selectDateRange?.order || "",
    is_active: selectDateRange?.is_active ?? true,
  };

  const validationSchema = Yup.object().shape({
    name: Yup.string().required("Name is required"),
    type: Yup.string().required("Type is required"),
    value: Yup.string().required("Counts is required"),
  });
  ...
  const handleSubmit = async (
    ...
    try {
      if (selectDateRange) {
        values.uid = selectDateRange.uid;
      }
      const result = await createUpdateDateRange(values);
          ...
          <div className="modal-content">
            <div className="modal-header">
              <h5 className="modal-title" id="exampleModalLabel3">
                {selectDateRange === null ? "Create New" : "View / Update"} Date
                Ranges{" "}
              </h5>
              <button
                type="button"
                        ...
                        <Select
                          ...
                          value={
                            dateRangeData
                              ...
                              .find((option) => option.value === values.type) ||
                            null
                          }
                          isClearable
                        />
