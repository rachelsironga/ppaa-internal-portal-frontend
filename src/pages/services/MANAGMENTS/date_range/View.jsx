...
import "animate.css";
import { deleteDateRange, getDateRanges } from "./Queries";
import { DateRangeContext } from "../../../../utils/context";
import DateRangesModal from "./Modal";
import BreadCumb from "../../../../layouts/BreadCumb";

export const DateRangePage = () => {
  const pageSizeData = [5, 10, 20, 50, 70, 100];
  const [DateRanges, setDateRanges] = useState([]);
  const [loading, setLoading] = useState(true);
  const [error, setError] = useState(null);
  const [selectDateRange, setSelectedDateRange] = useState(null);
  ...
  const handleFetchData = async () => {
    ...
    try {
      ...
      if (result.status === 200 || result.status === 8000) {
        ...
        setError(true);
        showToast("No Date Range Found", "warning", "Fetch Completed");
      }
    } catch (err) {
      setError(true);
      showToast("Unable to Fetch Date Ranges", "warning", "Failed");
    } finally {
      setLoading(false);
    }
  };

  const handleDelete = async (dateRange = null) => {
    if (!dateRange) {
      Swal.fire("Error!", "Unable to Select this Date Range.", "error");
      return;
    }
    ...
    try {
      ...
      if (confirmation.isConfirmed) {
        const result = await deleteDateRange(dateRange.uid);
        if (result.status === 200 || result.status === 8000) {
          Swal.fire(
            "Process Completed!",
            "The Date Range has been deleted.",
            "success"
          );
          handleFetchData();
        } else {
          console.error("Error deleting Date Range:", result);
    ...
    setSelectedDateRange(null); // Reset selected DateRange after deletion
  };
  // Fetch DateRanges on initial load
  useEffect(() => {
  ...
  return (
    ...
    >
      <BreadCumb pageList={["Settings", "Date Ranges"]} />

      <div className="card">
        <div className="d-flex justify-content-between align-items-center card-header">
          <h5 className="mb-0">All Date Range</h5>
          <DateRangesModal
            title="View Date Ranges"
            ...
            <table className="table table-hover table-align-middle mb-0 table-bordered">
              ...
              <tbody className="table-border-bottom-0">
                {loading ? (
                    ...
                    <td colSpan="100%">
                      <div className="col-md-12 col-lg-12 col-sm-12 p-2">
                        ...
                        <center className="mt-1">
                          <h6 className="text-muted">Fetching Date Ranges</h6>
                        </center>
                      </div>
                    </td>
                  </tr>
                ) : error || DateRanges.length === 0 ? (
                  <tr>
                    <td colSpan="100%">
                  ...
                  DateRanges.map((dataRows, index) => (
                    <tr key={dataRows.uid}>
                      <td>{(currentPage - 1) * pageSize + index + 1}</td>
                      <td className="fw-medium">{dataRows.name}</td>
                      <td className="fw-medium">{dataRows.value}</td>
