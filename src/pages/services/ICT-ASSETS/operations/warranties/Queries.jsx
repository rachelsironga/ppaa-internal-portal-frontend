...
export const createUpdateAsset = async (assetData) => {
    try {
        ...
        const url = isUpdate
            ? `${API_BASE_URL}/api/asset-warranties/${assetData.uid}/update`
            : `${API_BASE_URL}/api/asset-warranties/create`;

        const response = await api[method](url, assetData, config);
        return response.data;
    } catch (error) {
        console.error(`Error while ${assetData.uid ? 'updating' : 'creating'} warranty:`, error);
        throw error;
    }
};
...
export const deleteAsset = async (uid) => {
    try {
        const response = await api.delete(`${API_URL}/asset-warranties/${uid}/delete`);
        return response.data;
    } catch (error) {
