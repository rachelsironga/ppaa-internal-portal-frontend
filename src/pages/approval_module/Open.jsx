...
export const ApprovalModuleOpenPage = () => {
    const { uid } = useParams();
    const [approvalModule, setApprovalModule] = useState([]);
    ...
    const handleFetchData = async () => {
        ...
        try {
            ...
            if (result.status === 200 || result.status === 8000) {
                setApprovalModule(result.data);
                setSelectedApprovalModule(result.data)
    ...
    const handleSaveSorting = async () => {
        if (!isSorted || moduleLevels.length === 0) {
            Swal.fire("Process Failed", "Unable to Save Sorting. No Change found.", "error");
            return;
        }
        ...
        >
            <h4 className="py-3 mb-4">
                <span className="text-muted fw-light">Setting / <a
                    className="text-link"
                    href="/settings/approval-modules"
                >Approval Modules</a> /  </span> View
            </h4>
            ...
            <div className="card mb-4 animate__animated animate__fadeInDown animate__faster">
                ...
                <div className="card-body animate__animated animate__fadeInUp animate__faster">
                    ...
                    ) : error || approvalModule.length === 0 ? (
                        // error || approvalModule.length === 0
                        <div className="alert alert-info" role="alert">
                            <div className="alert-body text-center">
                                <p className="mb-0">Sorry! Unable to get Module Details Please Contanct System Administrator </p>
                            </div>
                        </div>
                    ) : (
                        <div className="col-md-12">
                            <div className="row mb-3">
                                ...
                                <div className="col-md-9 col-sm-12" >
                                    <p className="text-justify">{approvalModule?.name || ""}</p>
                                </div>
                            </div>
                            <div className="row mb-3">
                                <div className="col-md-1 col-sm-12" style={{ minWidth: "120px" }}>
                                ...
                                <div className="col-md-9 col-sm-12">
                                    <p className="text-justify">{approvalModule?.description || ""}</p>
                                </div>
                            </div>
                        </div>
                    )}
                                                    ...
                                                    >
                                                        {(provider, snapshot) => (
                                                            ...
                                                            >
                                                                ...
                                                                <td className="fw-medium">{dataRows.action.name}</td>
                                                                ...
                                                                <td {...provider.dragHandleProps}>
                                                                    <i className="bx bx-chevrons-up ms-1" style={{ cursor: "pointer" }} ></i>
